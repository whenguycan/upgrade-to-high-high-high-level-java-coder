...
#### 3、将score.sql导入到数据库中，并新增两条userId为5的记录
...
- sql映射文件的代理接口文件
  ...
  public interface ScoreMapper {
      List<ScoreEntity> selectByUserId(@Param("userId") Integer userId);
  }
  ```
...
public class TeEntity {
    ...
    private List<ScoreEntity> scoreInfo; //这儿是新增出来的，用于存放用户的考试成绩
}
```
...
<mapper namespace="com.example.demo11.mappers.TeMapper">
    <resultMap id="allcol" type="com.example.demo11.entity.TeEntity">
      ...
      4. ofType是指定select配置的方法返回的结果集合中什么类型的实体
      -->
      <collection property="scoreInfo" column="id"  ofType="com.example.demo11.entity.ScoreEntity" select="com.example.demo11.mappers.ScoreDao.selectByUserId">
        		<!-- 属性挨个赋值，会自动遍历 idea可能不识别没关系-->
            <id column="id" property="id"></id>
