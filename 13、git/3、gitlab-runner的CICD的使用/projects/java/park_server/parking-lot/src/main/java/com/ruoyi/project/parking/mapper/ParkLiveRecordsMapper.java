...
public interface ParkLiveRecordsMapper extends BaseMapper<ParkLiveRecords> {
    ...
    @Update("<script>" +
            "UPDATE t_park_live_records_parking_order t " +
            "<set> " +
            "<if test=\"payableAmount != null\">t.payable_amount = #{payableAmount},</if> " +
            "<if test=\"payAmount != null\">t.pay_amount = #{payAmount},</if> " +
            "<if test=\"discountAmount != null\">t.discount_amount = #{discountAmount},</if> " +
            "t.effect_flag=1,t.update_time = NOW() " +
            "</set>" +
            "WHERE t.order_no= #{orderNo} " +
            "</script>")
    int updareParkLiveParkingOrderByOrderNo(Double payableAmount, Double payAmount,Double discountAmount,String orderNo);

    List<HistoryParkingOrderVO> selectHistoryParkingOrderVO(ParkLiveRecordsParam parkLiveRecordsParam);


    @Delete("DELETE FROM t_park_live_records_parking_order t WHERE t.park_live_id = #{liveId}")
    boolean clearOrderNoWithLiveId(Integer liveId);
