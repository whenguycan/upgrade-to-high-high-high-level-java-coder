 ...
 * @since 2023-03-10
 */
public interface TDeductionOrderMapper extends BaseMapper<TDeductionOrder> {

    @Select("SELECT t1.park_live_id,t1.park_no,t1.order_type,t1.order_no, " +
            "t1.deduction_type AS unusual_type,t1.deduction_reason AS unusual_reason,t1.remark, " +
            "t2.car_number,t2.car_type,t4.name AS car_type_name, " +
            "t2.entry_time,t2.exit_time, " +
            "t3.payable_amount,t3.pay_amount " +
            "FROM t_deduction_order t1 " +
    ...
    List<UnusualOrderVO> queryUnusualOrderVOCondition(@Param(Constants.WRAPPER) QueryWrapper<TDeductionOrder> qw);
}
