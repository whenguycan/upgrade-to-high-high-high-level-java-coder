...
public class BParkChargeRelationHolidayServiceImpl implements IBParkChargeRelationHolidayService
{
    ...
    {
        int count = bParkChargeRelationHolidayMapper.deleteBParkChargeRelationHolidayByIds(ids);
        // 刷新收费规则关联
        if (count > 0) {
    ...
    {
        int count = bParkChargeRelationHolidayMapper.deleteBParkChargeRelationHolidayById(id);
        if (count > 0) {
            pCGrpcClientService.refreshParkLotChargeRelation(SecurityUtils.getParkNo());
        }
        return count;
