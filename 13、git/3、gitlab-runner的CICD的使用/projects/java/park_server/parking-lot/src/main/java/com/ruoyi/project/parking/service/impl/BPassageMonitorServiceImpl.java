...
public class BPassageMonitorServiceImpl implements BPassageMonitorService {
    ...
    @Autowired
    private BSettingRegularCarCategoryServiceImpl bSettingRegularCarCategoryService;
    ...
    public boolean openByOpenType(String carNumber, String parkNo, Integer passageId, String carType) {
        ...
        //余位
        Integer remainder = getSpaceRemain(CacheConstants.PARKNO_ACCOUNT_KEY + parkNo);
        ...
        //不在黑白名单内
        //车辆类型不匹配
        if (!StringUtils.isNullOrEmpty(bSettingLot.getBandCarTypes()) && !bandCarTypes.contains(carType)) {
            return false;
        }
...
//                BSettingRegularCarCategory bSettingRegularCarCategory=bSettingRegularCarCategoryPriceService.getById(regularCarList.get(0).getCarCategoryId());
                List<String> bandRegularCodes = Arrays.asList(bPassage.getBandRegularCodes().split(","));
                if (bandRegularCodes.contains(regularCarList.get(0).getCarCategoryId().toString())) {
                    return true;
                }
            }
        }
    ...
    private boolean judgeWhite(String carNumber, String parkNo) {
        ...
        List<WhiteList> list = whiteListService.listByParkNoAndCarNumberUnsafe(whiteList);
        if (list.size() > 0) {
            return true;
        }
        return false;
