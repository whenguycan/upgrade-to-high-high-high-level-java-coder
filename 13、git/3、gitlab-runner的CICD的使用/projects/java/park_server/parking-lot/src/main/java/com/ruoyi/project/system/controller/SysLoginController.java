...
public class SysLoginController {
     ...
     * @return 路由信息
     */
    @GetMapping("getRoutersChange")
    public AjaxResult getRoutersChange() {
        //Long userId = SecurityUtils.getUserId();
        List<SysMenu> menus = menuService.selectMenuTreeByRoleId(2L);
        return AjaxResult.success(menuService.buildMenus(menus));
    }
     ...
     */
    @PostMapping("/reLogin")
    public AjaxResult reLogin(@RequestBody LoginBody loginBody) {
        AjaxResult ajax = AjaxResult.success();
        // 生成令牌
        LoginResult loginResult = loginService.reLogin(loginBody);
        ajax.put(Constants.TOKEN, loginResult.getToken());
        ajax.put(Constants.REFRESH_TOKEN, loginResult.getRefreshToken());
        return ajax;
    }
