...
public class TCouponTypeController extends BaseController {
    ...
    @PostMapping("/add")
    public AjaxResult add(@RequestBody @Valid TCouponType tCouponType) {
        tCouponType.setUserId(SecurityUtils.getUserId());
    ...
    @PutMapping("/edit")
    public AjaxResult edit(@RequestBody @Valid TCouponType tCouponType) {
        int result = tCouponTypeService.updateTCouponType(tCouponType);
        if (result == -1) {
            return error("此类优惠券已被购买");
        }
        return toAjax(1);
    }
    ...
    @ApiOperation(value = "查询优惠券种类列表", notes = "null")
    @GetMapping("/list_pc")
    public TableDataInfo list_pc(TCouponType tCouponType) {
        startPage();
        tCouponType.setParkNo(SecurityUtils.getParkNo());
        List<TCouponType> list = tCouponTypeService.selectTCouponTypeListPC(tCouponType);
        int total = tCouponTypeService.selectTCouponTypeListPCCount(tCouponType);
        return getDataTable(list, Long.valueOf(String.valueOf(total)));
    }
}
