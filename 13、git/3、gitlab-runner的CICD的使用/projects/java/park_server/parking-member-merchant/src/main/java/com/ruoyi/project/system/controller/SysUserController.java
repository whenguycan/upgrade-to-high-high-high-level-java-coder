...
public class SysUserController extends BaseController {
    ...
    @Autowired
    private ITCouponTypeService itCouponTypeService;
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user) {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkUserNameUnique(user))) {
            ...
            if (UserConstants.NOT_UNIQUE.equals(userService.checkMerchantPhoneUnique(user))) {
        ...
        List<TCouponType> types = itCouponTypeService.selectCouponsByParkNo(SecurityUtils.getParkNo(), "1");
        if(!types.isEmpty()){
            for (TCouponType type : types){
                type.setCreateTime(DateUtils.getNowDate());
                type.setCreateBy(SecurityUtils.getUsername());
                type.setUserId(userid);
                tCouponTypeMapper.insertTCouponType(type);
            }
        }
        return toAjax(userid.intValue());
    }
