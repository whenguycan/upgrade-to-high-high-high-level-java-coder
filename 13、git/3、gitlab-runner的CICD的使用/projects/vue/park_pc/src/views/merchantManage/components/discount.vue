  ...
  <div class="discount">
    <div class="content">
      ...
      <el-table v-loading="loading" :data="discountList">
        <el-table-column label="充值金额（元）" prop="chargeAmount">
          <template slot-scope="scope">
            <el-input v-model="scope.row.chargeAmount" />
          </template>
        </el-table-column>
        <el-table-column label="优惠金额（元）" prop="giveAmount">
          <template slot-scope="scope">
            <el-input v-model="scope.row.giveAmount" />
          </template>
        </el-table-column>
      ...
      <div class="btn-group" v-if="discountList.length != 0">
...
import { listDiscount, addDiscount } from "../api";
...
export default {
  ...
  methods: {
    ...
    submit() {
      // 判断当前list中有无空数据
      const check = this.discountList.find(item => item.chargeAmount == '' || item.giveAmount == '')
      if(!!check) {
