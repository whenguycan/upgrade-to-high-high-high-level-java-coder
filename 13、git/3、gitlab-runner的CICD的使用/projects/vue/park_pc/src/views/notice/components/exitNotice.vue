 ...
 * @LastEditors: cuijing
 * @LastEditTime: 2023-03-10 16:20:58
 * @文件相对于项目的路径: \park_pc\src\views\notice\components\exitNotice.vue
-->
<template>
  <div class="pushCon">
    ...
    <div class="pushConBot" v-if="notice.isEdit">
      ...
      <div class="temLi">
        <el-input v-model="notice.firstData" type="textarea" :rows="2" placeholder="请输入消息开始内容，例：尊敬的会员，欢迎使用融驿停！" maxlength="10" show-word-limit></el-input>
      </div>
      <div class="temLi">
        <span class="temTit">停车场名:</span>
        <span class="temCon">XX停车场</span>
      </div>
      <div class="temLi">
        <span class="temTit">车牌号码:</span>
        <span class="temCon">苏D:XXXXXX</span>
      </div>
      <div class="temLi">
        <span class="temTit">离场时间:</span>
        <span class="temCon">2022-02-06 13:53:00</span>
      </div>
      <div class="temLi">
        <span class="temTit">停车时长:</span>
        <span class="temCon">0天1时30分12秒</span>
      </div>
      <div class="temLi">
        <span class="temTit">停车费用:</span>
        <span class="temCon">￥10.00元</span>
      </div>
      <div class="temLi">
        <el-input v-model="notice.remarkData" type="textarea" :rows="2" placeholder="请输入消息结束内容，例：融驿停感谢您的使用，期待下次光临！" maxlength="20" show-word-limit></el-input>
      </div>
  ...
  data () {
    return {
      notice: {
        name: '出场通知',
        status: false,
        templateId:'dv6FxR94QlbrqnFYclOVxS1TwQ_fG0mW_AT8tvBY0h8',
        firstData: '',
        remarkData: '',
        isEdit: false,
      }
      ...
      if(res.code == 200){
        if(isOpen){
          if(this.notice.status){
            this.$message.success("开启出场通知成功！")
          }else{
            this.$message.success("关闭出场通知成功！")
          }
        }else{
          this.$message.success("编辑出场通知成功！")
        }
        ...
        if(isOpen){
          if(this.notice.status){
            this.$message.error("关闭出场通知失败！")
          }else{
            this.$message.success("关闭出场通知失败！")
          }
        }else{
          this.$message.error("编辑出场通知失败！")
        }
        this.notice.isEdit = false
        this.$emit('isEdit',true)
      }
