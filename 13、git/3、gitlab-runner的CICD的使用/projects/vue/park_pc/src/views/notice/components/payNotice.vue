 ...
 * @LastEditors: cuijing
 * @LastEditTime: 2023-03-10 16:25:45
 * @文件相对于项目的路径: \park_pc\src\views\notice\components\payNotice.vue
-->
<template>
  <div class="pushCon">
    ...
    <div class="pushConBot" v-if="notice.isEdit">
      ...
      <div class="temLi">
        <el-input v-model="notice.firstData" type="textarea" :rows="2" placeholder="请输入消息开始内容，例：您好，您已完成支付该笔订单，请知悉。" maxlength="10" show-word-limit></el-input>
      </div>
      <div class="temLi">
      ...
      <div class="temLi">
        <span class="temTit">支付时间:</span>
        <span class="temCon">2022-02-06 12:45:00</span>
      </div>
      <div class="temLi">
        <span class="temTit">支付金额:</span>
        <span class="temCon">￥10.00元</span>
      </div>
      <div class="temLi">
        <el-input v-model="notice.remarkData" type="textarea" :rows="2" placeholder="请输入消息结束内容，例：您已完成支付，请在30分钟内离场。" maxlength="20" show-word-limit></el-input>
      </div>
  ...
  data () {
    return {
      notice: {
        name: '缴费通知',
        status: false,
        templateId:'slmfRxcvlCxlYrH0Z6Mp3pASpkanDaebkKZBf3wnoL8',
        firstData: '',
        remarkData: '',
        isEdit: false,
      }
      ...
      if(res.code == 200){
        if(isOpen){
          if(this.notice.status){
            this.$message.success("开启缴费通知成功！")
          }else{
            this.$message.success("关闭缴费通知成功！")
          }
        }else{
          this.$message.success("编辑缴费通知成功！")
        }
        ...
        if(isOpen){
          if(this.notice.status){
            this.$message.error("关闭缴费通知失败！")
          }else{
            this.$message.success("关闭缴费通知失败！")
          }
        }else{
          this.$message.error("编辑缴费通知失败！")
        }
        this.notice.isEdit = false
        this.$emit('isEdit',true)
      }
