...
public class ParkingFeeUtil {
    ...
    public static BigDecimal chargeParkingFee(long durations, long minUnit, BigDecimal rate, String roundMode) {
        ...
        // 根据舍入方式，取余数
        BigDecimal remainder = BigDecimal.valueOf(durations).divide(BigDecimal.valueOf(minUnit), 0, way);
        // 乘以费率就是这期间的停车费
        return remainder.multiply(rate);
    }
    ...
    public static BigDecimal chargeParkingFee(long durations, long minUnit, BigDecimal rate, String roundMode,
                                              ...
                                              BigDecimal existFee) {
        // 存在区间限价
        boolean existLP = limitPrice != null && limitPrice.compareTo(BigDecimal.ZERO) > 0;
     ...
     * @return java.math.BigDecimal
     */
    public static BigDecimal divideHideDuration2ChargeFee(ParkingRecord parkingRecord, long duration, String chargeContainFT, BigDecimal ceilingPrice) {
        // 隐藏的分割，不会考虑免费时长次数、首时段计费方式
        List<ParkingRecord> dRecords = divideDuration(parkingRecord, duration,
        ...
        // 如果算费不包含免费时长，最后一条记录需要根据传入的停车记录的免费时长使用进行配置，否则会遗漏掉免费时间
        if (YesOrNo.NO.getValue().equals(chargeContainFT)) {
            dRecords.get(dRecords.size()-1).setUseFreeTime(parkingRecord.isUseFreeTime());
        }
        return divisionChargeFee(dRecords, ceilingPrice);
    }
