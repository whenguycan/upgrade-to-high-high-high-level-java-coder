...
@Slf4j
@Component
@ServerEndpoint(value = "/websocket/Entry/{SentryBoxName}",encoders = { ServerEncoderEntry.class })
public class WebSocketEntryServer {
    /**
     * 建立连接成功调用 (Session + 场景)
     */
    @OnOpen
    public void onOpen(Session session, @PathParam("SentryBoxName") String SentryBoxName) throws IOException {
        log.info("[onOpen][session({}) 接入, [SentryBoxName: {}]", session, SentryBoxName);
        WebSocketServerEntryPool.addDataConnect(session, SentryBoxName);
    }
