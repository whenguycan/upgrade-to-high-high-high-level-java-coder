...
public class RabbitmqConsumer {
    ...
    public void consumerEntryMsg(String message) {
        ...
        notificationRecord.setCreateBy(notificationRecord.getUserPhone());
        ...
        if (responseEntity.getStatusCode().equals(HttpStatus.OK)) {
            ...
            if ((Integer) jsonObject.get("errcode") == 0) {
                notificationRecord.setMessageId((String) jsonObject.get("msgid"));
    ...
    public void consumerExitMsg(String message) {
        ...
        log.info("记录通知日志...");
        //记录通知日志
        NotificationRecord notificationRecords = new NotificationRecord();
        notificationRecords.setParkNo(data.getParkNo());
        notificationRecords.setSendTime(NotificationRecordEnums.SendTime.EXIT.getValue());
        notificationRecords.setUserPhone(getMemberByCarResponse.getPhonenumber());
        notificationRecords.setNotifyTime(LocalDateTime.now());
        notificationRecords.setComment(buildExitComment(wechatNotificationData));
        notificationRecords.setStatus(NotificationRecordEnums.Status.WAIT.getValue());
        notificationRecords.setCreateBy(notificationRecords.getUserPhone());
        notificationRecords.setCreateTime(LocalDateTime.now());
        notificationRecordService.save(notificationRecords);
        log.info("记录通知日志完成，{}", notificationRecords);
        //构建微信推送模板消息请求
        log.info("构建微信推送模板消息请求...");
        WechatNotificationRequest wechatNotificationRequest = new WechatNotificationRequest();
        wechatNotificationRequest.setTouser(getMemberByCarResponse.getOpenId());
    ...
    @RabbitListener(bindings = @QueueBinding(
            ...
            exchange = @Exchange(value = RabbitmqConstant.EXCHANGE_PARKING_PAYMENT, type = ExchangeTypes.TOPIC),
    ...
    public void consumerPayMsg(String message) {
        ...
        NotificationRecord notificationRecords = new NotificationRecord();
        notificationRecords.setParkNo(data.getParkNo());
        notificationRecords.setSendTime(NotificationRecordEnums.SendTime.PAY.getValue());
        notificationRecords.setUserPhone(getMemberByCarResponse.getPhonenumber());
        notificationRecords.setNotifyTime(LocalDateTime.now());
        notificationRecords.setComment(buildPayComment(wechatNotificationData));
        notificationRecords.setStatus(NotificationRecordEnums.Status.WAIT.getValue());
        notificationRecords.setCreateBy(notificationRecords.getUserPhone());
        notificationRecords.setCreateTime(LocalDateTime.now());
        notificationRecordService.save(notificationRecords);
