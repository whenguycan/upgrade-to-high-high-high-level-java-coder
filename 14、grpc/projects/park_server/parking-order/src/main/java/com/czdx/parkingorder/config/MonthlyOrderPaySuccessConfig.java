...
public class MonthlyOrderPaySuccessConfig {
    ...
    public CustomExchange orderExchange(){
        ...
        //4. 交换机是否需要自动删除
        //5. 其它配置参数
        return new CustomExchange(RabbitmqConstant.ORDER_PAYSUCCESS_EXCHANGE, "topic", true, false);
    }

    //声明一个Queue
    @Bean
    public Queue merchantQueue(){
        return new Queue(RabbitmqConstant.MERCHANT_ORDER_QUEUE);
    }
    ...
    public Binding merchantBinding(
        ...
        return BindingBuilder.bind(queue).to(customExchange).with(RabbitmqConstant.MERCHANT_ORDER_ROUTING_KEY).noargs();
    }

    @Bean
    public Queue monthlyQueue(){
        return new Queue(RabbitmqConstant.MONTHLY_ORDER_QUEUE);
    }
    ...
    public Binding monthlyBinding(
            ...
            @Qualifier("orderExchange") CustomExchange customExchange){
        return BindingBuilder.bind(queue).to(customExchange).with(RabbitmqConstant.MONTHLY_ORDER_ROUTING_KEY).noargs();
    }
