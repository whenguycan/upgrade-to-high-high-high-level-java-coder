## springboot3.x+shardingsphere+mybatis-plus实现水平分库 + 水平分表
        ...
        tables:
          t_order:
            ...
            table-strategy: #数据表数据的分片策略
              standard:
                sharding-column: num #参与计算分表的字段
                sharding-algorithm-name: tetete  #分表的策略算法名称

            actual-data-nodes: ds1.order_$->{1..2}, ds2.order_$->{1..2} #逻辑表有哪些数据节点，数据节点由数据源名称.真实表 得到
